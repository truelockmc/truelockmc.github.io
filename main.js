...
function makeCard(cfg, idx, pinned) {
    ...
    const badge = pinned ? '<span class="pin-badge">&#x2605; featured</span>' : "";
    const tag   = cfg.tag || (cfg.language || "Web") + " &middot; GitHub Pages";

    a.innerHTML =
        badge +
        '<div class="card-icon ci-' + cfg.ci + '">' + cfg.emoji + "</div>" +
        '<div class="card-tag">// ' + tag + "</div>" +
        '<div class="card-title">' + cfg.title + "</div>" +
        '<p class="card-desc">' + cfg.desc + "</p>" +
...
function makeRepoCard(repo, idx) {
    ...
    /* Social preview – GitHub generates OG images for every repo */
    ...
    const langHtml = repo.language
        ? '<span class="repo-lang"><span class="lang-dot" style="background:' +
          langColor(repo.language) + '"></span>' + repo.language + "</span>"
        : "";

    a.innerHTML =
        /* preview image */
        '<div class="repo-preview">' +
        '<img src="' + imgSrc + '" alt="' + repo.name + ' preview" ' +
        'onerror="this.parentElement.innerHTML=\'<div class=repo-preview-placeholder>' +
        repo.name.toUpperCase() + "</div>'\" loading=\"lazy\"></div>" +
        /* body */
        '<div class="repo-body">' +
            '<div class="repo-meta">' +
                '<span class="repo-tag">// ' + (repo.language || "Web") + " &middot; github.com</span>" +
                ...
                "</span>" +
            "</div>" +
            '<div class="repo-title">' + repo.name.replace(/-/g, " ") + "</div>" +
            '<p class="repo-desc">' + (repo.description || "No description provided.") + "</p>" +
            (topicsHtml ? '<div class="repo-topics">' + topicsHtml + "</div>" : "") +
            '<div class="repo-footer">' + langHtml + '<span class="repo-arrow">View on GitHub</span></div>' +
        "</div>";
...
async function loadPages(repos) {
    ...
    const totalPages  = PINNED.length + dynamic.length + (rootRepo?.has_pages ? 1 : 0);
